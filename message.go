...
func (cm *CeleryMessage) reset() {
	cm.Headers = nil
	cm.Body = ""
	cm.Properties.CorrelationID = stringutil.UUID().String()
	cm.Properties.ReplyTo = stringutil.UUID().String()
	cm.Properties.DeliveryTag = stringutil.UUID().String()
}
	...
	New: func() interface{} {
		return &CeleryMessage{
			...
			Properties: CeleryProperties{
				BodyEncoding:  "base64",
				CorrelationID: stringutil.UUID().String(),
				ReplyTo:       stringutil.UUID().String(),
				...
				DeliveryMode: 2,
				DeliveryTag:  stringutil.UUID().String(),
			},
